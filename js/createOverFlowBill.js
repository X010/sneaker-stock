...
var opt = {
    ...
    "template": [
        ...
        },
        {
            ...
            "type": true,
            "class" : "spec",
            "template": "<span class='readonly' id='rtgunit_#{index}'>#{unit}</span>",
            "dattr": []
        },
        {
            "name": "volume_price",
...
$(function () {
    ...
    if (action == 'co') {
        ...
        var data = inventoryRes.findOverFlowById(orderId);
        if (data != null) {
            ...
            $("#createOverFlowBill").hide();
            $("#checkOverFlowBill").show();
            stampStatus(id2text(showInStatusList, data.status));
            $('#btn-list').click(function(){
                openParentForFrame('报溢单','/mainframe/inventory/overflowBill.html?option='+data.status, 361);
            });
        }
    }
    /*
...
function createOverFlow() {
    ...
    if (goodList.length > 0) {
        ...
        var sres = inventoryRes.addOverFlow(postData);
        if (sres != null) {
            noticeFrame(361, 'refrush', page);
            runnerConfiremUrl("操作提示", "操作成功", false, "/mainframe/inventory/createOverFlowBill.html?iframeid=36&iframename=" + encodeURI("新建报溢单"));
        }
    }
    else {
        runnerAlert("操作提示", "请添加商品");
...
function editAndCheckOverFlow() {
    ...
    if (goodList.length > 0) {
        ...
        var sres = inventoryRes.updateAndCheakOverFLowBill(currOrderId, postData);
        if (sres != null) {
            noticeFrame(361, 'refrush', page);
...
function deleteOverFlow() {
    if (currOrderId != null && currOrderId.length > 0) {
        var sres = inventoryRes.deleteOverFlowBill(currOrderId);
        if (sres != null) {
            noticeFrame(361, 'refrush', page);
 ...
 * 创建并审核报溢单
 */
function createAndCheckOverFlow() {
    //获取table中的商品列表
    var goodList = getTableData();
    if (goodList.length > 0) {
        var in_sid = $("#transferOut").val();
        ...
        var sres = inventoryRes.addAndCheckOverFLowBill(postData);
        if (sres != null) {
            noticeFrame(361, 'refrush', page);
            runnerConfiremUrl("操作提示", "审核通过", false, "/mainframe/inventory/createOverFlowBill.html?iframeid=36&iframename=" + encodeURI("新建报溢单"));
        }
    }
    else {
        runnerAlert("操作提示", "请添加商品");
