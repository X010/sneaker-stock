...
$(function () {
    ...
    if (!checkPower(83)){
        ...
        $('#btn-create').click(function(){
            openParentForFrame('新建收款单', "/mainframe/finance/createGathering.html?iframeid=83&iframename=" + encodeURI("新建收款单"), 83);
        });
    }
...
function fullTable(data) {
    if (data != null && data != 'undefined' && data.data != null && data.data.length > 0) {
        ...
        for (var i = 0; i < data.length; i++) {
            var rowContent = "";
            if (data[i].status == 1) {
                rowContent += "<tr ondblclick='openParentForFrame(\"" + simpleBillNo(data[i].id, "未审") + "\",\"/mainframe/finance/createGathering.html?page=" + currentPage + "&action=ch&id=" + data[i].id + "\","+data[i].id+")'>";
            } else if (data[i].status == 2) {
                rowContent += "<tr ondblclick='openParentForFrame(\"" + simpleBillNo(data[i].id, "已审") + "\",\"/mainframe/finance/createGathering.html?page=" + currentPage + "&action=xd&id=" + data[i].id + "\","+data[i].id+")'>";
            } else {
                rowContent += "<tr ondblclick='openParentForFrame(\"" + simpleBillNo(data[i].id, "查看") + "\",\"/mainframe/finance/createGathering.html?page=" + currentPage + "&action=vw&id=" + data[i].id + "\","+data[i].id+")'>";
            }
