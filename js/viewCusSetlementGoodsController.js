...
function handleGoodsList(goodsList){
    ...
    $("#cusSetlementGoods").runnerTableOnStart(opts, goodsList, bindAutoComplete);
}
...
$(function () {
    ...
    if (msg) {
        ...
        if (id && msg) {
            currentId = id;
            var data = financeRes.findCusSetlementGoodsById(id);
            if (data != null) {
                //console.log(data);
                $("#fullperson").html(data.uname); //填单人
                $("#cuname").html(fieldNull(data.cuname, '(空)')); //审核人
                $("#setlementId").html(id);
                $("#customer").html(fieldNull(data.ccname)).attr('disabled', 'disabled');
                ...
                //总计
                $("#amountTobe").html(formatAmountWithComma(parseFloat(data.amount_price) + parseFloat(data.discount_price)));
                $("#amountCut").html(formatAmountWithComma(data.discount_price));
