...
(function ($) {
     ...
     * @param type company / system 公司商品分类 OR 系统商品分类
     */
    $.fn.treeGoods = function (opts, open, type) {
        //整理参数
        open = open ? open : [];
        ...
        var root_type;
        if (type == 'system') {
            GTR = new restGoodsRepository();
            root_type = GTR.readGoodsTypeTree();
        } else {
            GTR = new goodsTypeRepository();
            root_type = GTR.readTypeTree();
        }
        var zNodes = root_type ? root_type : [];
        zNodes.unshift({ id:0, pId:-1, name:"商品分类", open:true}); //添加个超级根
...
function beforeRemove(treeId, treeNode) {
    ...
    var flag_delete_type = confirm("确定删除分类 [" + treeNode.name + "] 吗？");
...
function addHoverDom(treeId, treeNode) {
    ...
    var addStr = "<span class='button add' id='" + treeNode.tId + '_add' + "' title='新建子分类' onfocus='this.blur();'></span>";
    sObj.after(addStr);
    btn = $("#"+treeNode.tId+"_add");
    if (btn) btn.bind("click", function(){
        var id;
        var name = '新分类' + (newCount++); //临时名称
        var pId = treeNode.id;
