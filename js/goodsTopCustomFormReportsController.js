...
function buildParams(){
    var params = {};
    if ($('#searchBox-customer').val() == ''){
        //runnerAlert('操作提示', '请指定客户');
        return false;
    } else {
        params['ccid'] = $('#searchBox-customer').val();
    }
    //先取小分类
    params['gtids'] = '';
...
function refrush(page) {
    ...
    if (params){
        var data = resRCR.query('REPORTS_FORM_CUSTOMER_GOODS_TOP_READ', params, page, defaultPageNum);
...
function saveExcel(){
    var params = buildParams(); //获取搜索条件
    var res = resRCR.download('REPORTS_FORM_CUSTOMER_GOODS_TOP_READ', params, 1, EXCEL_LINE_MAX);
...
function fullTable(res) {
    if (res != null && res.data != null) {
        ...
        if (data.length == 0) {
            ...
            tableContent += "<td class='' colspan='5'><div class='td-wrap'>总计</div></td>";

            tableContent += "<td class=''><div class='td-wrap'>" + num2total(data.sell_total) + "</div></td>";
