...
$(function () {
    ...
    if (!checkPower(63)){
        ...
        $('#btn-create').click(function(){
            openParentForFrame('新建出货价调整单', "/mainframe/price/createStockoutPrice.html?iframeid=63&iframename=" + encodeURI("新建出货价调整单"), 63);
        });
    }
...
function refrush(page) {
    ...
    if (1){
        var data = priceRes.findStockoutPriceByField(currentPage, defaultPageNum, params);
        fullTable(data);
...
function fullTable(data) {
    if (data != null) {
        data = data.data;
        $("#stockoutPriceList tbody").empty();
        $("#table-empty").hide();
        for (var i = 0; i < data.length; i++) {
            if (data[i].status == 1){
                var rowContent = "<tr ondblclick='openParentForFrame(\"" + simpleBillNo(data[i].id, "未审") + "\",\"/mainframe/price/createStockoutPrice.html?page=" + currentPage + "&action=ch&id=" + data[i].id + "\","+data[i].id+");'>";
            } else {
                var rowContent = "<tr ondblclick='openParentForFrame(\"" + simpleBillNo(data[i].id, "查看") + "\",\"/mainframe/price/createStockoutPrice.html?page=" + currentPage + "&action=see&id=" + data[i].id + "\","+data[i].id+");'>";
            }
            ...
            //rowContent += "<td><a class='btn-small btn-op' href='javascript:openParentForFrame(\"" + simpleBillNo(data[i].id, "SOP") + "\",\"/mainframe/price/createStockoutPrice.html?action=ch&id=" + data[i].id + "\")'>查看</a></td>";
            rowContent += "</tr>";
            $("#stockoutPriceList tbody").append(rowContent);
        }
    }
