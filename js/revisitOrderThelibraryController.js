...
$(function () {
    ...
    if (action == 'up' && orderId != null) {
        ...
        var data = stockRes.findOrderByIdForRevisit(orderId);
        if (data != null) {
            ...
            $("#suid").val(data.suid);
            $("#suname").val(addPhone2Name(data.suname, data.suphone));
            $("#memo").text(fieldNull(data.memo, '无'));
            $("#visit_memo").val(fieldNull(data.visit_memo));
            ...
            $('#visit_memo').removeAttr("disabled");
        }
    }
 ...
 * 保存回访结果
 */
function saveVisitMemo() {
    var visit_memo = $.trim($("#visit_memo").val());
    if (visit_memo == ''){
        runnerAlert("操作提示", "请填写回访结果");
        return;
    }
    if (!currOrderId){
    ...
    var sres = stockRes.saveRevisitForCustomOrder(currOrderId, postData);
    if (sres != null) {
        noticeFrame(25, 'refrush', page);
