...
function refrush(page) {
    ...
    if (1){
        var data = resRCR.query('REPORTS_FORM_SUPPLIER_TOP_READ', params, page, defaultPageNum);
...
function saveExcel(){
    var params = buildParams(); //获取搜索条件
    var res = resRCR.download('REPORTS_FORM_SUPPLIER_TOP_READ', params, 1, EXCEL_LINE_MAX);
    /*if (res && res.data && res.data.length){
        ...
        for (var i=0; i<data.length; i++){
            content += '\n';
            content += fieldNull(data[i].scname) + ',';
