...
function buildParams(){
    var params = {};
    if ($('#searchBox-supplier').val() == ''){
        //runnerAlert('操作提示', '请选择供应商');
        return false;
    } else {
        params['scid'] = $('#searchBox-supplier').val();
    }
    //先取小分类
    params['gtids'] = '';
...
function refrush(page) {
    ...
    if (params){
        var data = resRCR.query('REPORTS_FORM_SUPPLIER_GOODS_TOP_READ', params, page, defaultPageNum);
...
function saveExcel(){
    var params = buildParams(); //获取搜索条件
    var res = resRCR.download('REPORTS_FORM_SUPPLIER_GOODS_TOP_READ', params, 1, EXCEL_LINE_MAX);
