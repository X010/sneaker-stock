...
function refrush(page) {
    ...
    if (1){
        var data = resRCR.query('REPORTS_BASE_SALESMAN_TOP_CUSTOM_READ', params, page, defaultPageNum);
...
function saveExcel(){
    var params = buildParams(); //获取搜索条件
    var res = resRCR.download('REPORTS_BASE_SALESMAN_TOP_CUSTOM_READ', params, 1, EXCEL_LINE_MAX);
    /*if (res && res.data && res.data.length){
        ...
        for (var i=0; i<data.length; i++){
            ...
            content += num2total(data[i].count) + ',';
        }
        //总计
        content += '\n';
        content += '总计,';
        content += num2total(add_up.count) + ',';
        //保存
...
function fullTable(res) {
    if (res != null && res.data != null) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            tableContent += "<td class='num'><div class='td-wrap'>" + num2total(data[i].count) + "</div></td>";
            tableContent += "</tr>";
        ...
        if (data.length == 0) {
            ...
            tableContent += "<td class='num'><div class='td-wrap'>" + num2total(data.count) + "</div></td>";
            tableContent += "</tr>";
            $("#mainList tbody").append(tableContent);
        }
    }
