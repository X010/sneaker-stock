...
function refrush(page) {
    ...
    if (params){
        var data = resRCR.query('REPORTS_SALESMAN_GOODS_CUSTOM_BY_ORDER_READ', params, page, defaultPageNum);
...
function saveExcel(){
    var params = buildParams(); //获取搜索条件
    if (params){
        var res = resRCR.download('REPORTS_SALESMAN_GOODS_CUSTOM_BY_ORDER_READ', params, 1, EXCEL_LINE_MAX);
...
function fullTable(res) {
    if (res != null && res.data != null) {
        ...
        if (data.length == 0) {
            ...
            tableContent += "<td class='num'><div class='td-wrap'>" + num2price(data.box_total) + "</div></td>";
            ...
            tableContent += "<td class='num'><div class='td-wrap'>" + num2price(data.free_box_total) + "</div></td>";
            tableContent += "</tr>";
            $("#mainList tbody").append(tableContent);
        }
    }
