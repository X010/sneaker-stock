    ...
    if (msg) {
        ...
        if (action == null || action == "undefined" || action.length <= 0) {
            ...
            $('#btn-list').click(function () {
                openParentForFrame('收款单', '/mainframe/finance/gathering.html?option=1', 833);
            });
            $("#cuname").html('(空)');
            $("#checktime").html('(空)');
        ...
        if (id && action){
            ...
            if (data != null) {
                $('#btn-list').click(function () {
                    openParentForFrame('收款单', '/mainframe/finance/gathering.html?option=' + data.status, 833);
                });
...
function createGathering() {
    ...
    if (res != null) {
        noticeFrame(833, 'refrush', page);
        runnerConfiremUrl("操作提示", "保存成功", false, "/mainframe/finance/createGathering.html?iframeid=83&iframename=" + encodeURI("新建收款单"));
    }
}
...
function checkGathering(add) {
    ...
    if (res != null) {
        noticeFrame(833, 'refrush', page);
        if (add){
            runnerConfiremUrl("操作提示", "审核通过", false, "/mainframe/finance/createGathering.html?iframeid=83&iframename=" + encodeURI("新建收款单"));
        } else {
