...
$(function () {
    ...
    if (!checkPower(82)){
        ...
        $('#btn-create').click(function(){
            openParentForFrame('新建供应商结算单', "/mainframe/finance/checkSupSetlement.html?iframeid=82&iframename=" + encodeURI("新建供应商结算单"), 82);
        });
    }
...
function refrush(page) {
    ...
    if (1) {
        var data = financeRes.findSupSetlementAll(currentPage, defaultPageNum, params);
        fullTable(data);
...
function fullTable(data) {
    if (data != null && data != 'undefined' && data.data != null && data.data.length > 0) {
        data = data.data;
        $("#supSetlementList tbody").empty();
        $("#table-empty").hide();
        for (var i = 0; i < data.length; i++) {
            var rowContent = "";
            if (data[i].status == 1) {
                rowContent += "<tr ondblclick='openParentForFrame(\"" + simpleBillNo(data[i].id, "未审") + "\",\"/mainframe/finance/checkSupSetlement.html?page=" + currentPage + "&action=ch&id=" + data[i].id + "\","+data[i].id+")'>";
            } else if (data[i].status == 2) {
                rowContent += "<tr ondblclick='openParentForFrame(\"" + simpleBillNo(data[i].id, "已审") + "\",\"/mainframe/finance/checkSupSetlement.html?page=" + currentPage + "&action=xd&id=" + data[i].id + "\","+data[i].id+")'>";
            } else {
                rowContent += "<tr ondblclick='openParentForFrame(\"" + simpleBillNo(data[i].id, "查看") + "\",\"/mainframe/finance/checkSupSetlement.html?page=" + currentPage + "&action=vw&id=" + data[i].id + "\","+data[i].id+")'>";
            }
            ...
            rowContent += "<td class=''><div class='td-wrap'>" + fieldNull(data[i].snames) + "</div></td>";
            rowContent += "<td class=''><div class='td-wrap'>" + statusNumbers[data[i].status]+ "</div></td>"; //状态
            rowContent += "</tr>";
            $("#supSetlementList tbody").append(rowContent);
        }
